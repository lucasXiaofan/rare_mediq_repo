...
class Generator:
    ...
    def __init__(self, args, tokenizer, model, evaluator: Evaluator) -> None:
        ...
        self.use_triples = args.use_triples
    ...
    def _generate_queries_and_search(self, questions):
        ...
        for question in questions:
            ...
            if self.retrieval_corpus == "wikipedia":
                ...
                request_query = "http://172.16.34.21:7000//api/search?query=" + search_query + "&k=" + str(self.num_retrieval)
        ...
        for kdx in range(1):
            ...
            for query in gen_queries:
                if self.retrieval_corpus == "wikipedia":
                    ...
                    request_query = "http://172.16.34.21:7000//api/search?query=" + query + "&k=" + str(
                        self.num_retrieval)
                x = requests.get(request_query)
                jsobj = json.loads(x.text)
                documents = []
                ...
                if self.use_triples:
                    for jdx in range(len(jsobj['top_facts'][:self.num_retrieval])):
                        score = jsobj['top_facts'][jdx]['score']
                        text =  " ".join(jsobj['top_facts'][jdx]['fact']) + ".\n"
                        if score >= self.retrieval_threshold:
