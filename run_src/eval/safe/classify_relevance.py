...
- Option without specify the content(e.g., "option A", "choice C")


Instructions:
1. The following STATEMENT has been extracted from the broader context of the \
given RESPONSE, and is the result of model generate given the PROMPT.
2. Modify the STATEMENT by replacing vague references with the proper entities \
from the RESPONSE or PROMPT that they are referring to.
...
Example 5:
PROMPT:
Imagine you are a medical doctor answering questions on a medical entrance test. For each of the following multiple-choice questions, \
please not only select the correct answer but also provide a detailed reasoning for your choice.\n**Question**: A junior orthopaedic surgery resident is \
completing a carpal tunnel repair with the department chairman as the attending physician. During the case, the resident inadvertently cuts a flexor tendon. \
The tendon is repaired without complication. The attending tells the resident that the patient will do fine, and there is no need to report this minor complication\
 that will not harm the patient, as he does not want to make the patient worry unnecessarily. He tells the resident to leave this complication out of the operative report. \
...
RESPONSE:
Option B. As a medical professional, it is essential to maintain transparency and honesty in patient care. 
The attending physician's suggestion to conceal the error from the patient and the operative report is unethical and potentially harmful. 
The patient has the right to know about any complications that occurred during the surgery, even if they are minor and will not affect the outcome. .

REVISED STATEMENT:
The subject of the statement "Option B is the correct answer." is "Option B". From the \
PROMPT, we can see that "Option B" refer to "Disclose the error to the patient and put it in the operative report". Therefore \
"Option B" without specifying other information, making it a vague reference. It should be replaced \
by "Option B: Disclose the error to the patient and put it in the operative report", which is the full name. Thus, the revised response is:
```
Option (B), Disclose the error to the patient and put it in the operative report is the correct answer.
...
def main(
    prompt: str, response: str, atomic_fact: str, model: open_modeling.Model
  ...
  return atomic_fact, model_responses
