...
def check_sentence(prompt, response, sentence, atomic_fact, rater):
...
def classify_relevance_and_rate(
  ...
  with ThreadPoolExecutor() as executor:
    future_checks = [executor.submit(check_sentence, prompt, response, sentence_data['sentence'], sentence_data['atomic_facts'][0], rater) for sentence_data in sentences_and_atomic_facts]
...
def main(prompt: str, response: str, rater: open_modeling.Model, split_fact: bool) -> dict[str, Any]:
