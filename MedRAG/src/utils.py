...
def embed(chunk_dir, index_dir, model_name, **kwarg):
    ...
    corpus_name = chunk_dir.split("/")[-2]
    corpus_file = "/".join(chunk_dir.split("/")[:-1]) + "/" + "all_chunks.pickle"
    all_chunks = []
    ...
    with torch.no_grad():
        for fname in tqdm.tqdm(fnames):
            ...
            chunks = [item["title"] +": "+ item["content"] for item in texts]
    ...
    def __init__(self, retriever_name="ncbi/MedCPT-Query-Encoder", corpus_name="textbooks", db_dir="./corpus", **kwarg):
        ...
        if not os.path.exists(self.chunk_dir):
            ...
            if self.corpus_name == "statpearls":
                ...
                os.system("wget https://ftp.ncbi.nlm.nih.gov/pub/litarch/3d/12/statpearls_NBK430685.tar.gz --no-check-certificate -P {:s}".format(os.path.join(self.db_dir, self.corpus_name)))
